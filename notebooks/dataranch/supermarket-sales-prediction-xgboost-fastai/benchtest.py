...
def main():
    ...
    input_dir = f'{os.path.dirname(__file__)}/input/{PROJECT_NAME}'
    ...
    for col in df1.columns:
        if '%' in df1[col].to_string() or ',' in df1[col].to_string():
            df1[col + '_processed'] = df1[col].str.replace('%', '').str.replace(',', '')
            #Try to convert this new column to a numeric type
            try:
                df1[col + '_processed'] = df1[col + '_processed'].apply(pd.to_numeric, errors='coerce').dropna()
            except Exception:
                print(f'{col} can not be converted to a float!')
    t01 = time.time()
    ...
    # The original code uses a for loop to iterate over each column in the dataframe and checks if '%' or ',' exists in the column. 
    # It then creates a new column with the '%' and ',' removed and tries to convert this new column to numeric. 
    # If the conversion fails, it prints a message. 
    # This approach is not efficient because it uses a for loop and applies operations on each column separately.
    # Instead, we can use vectorized operations provided by pandas to perform these operations on the entire dataframe at once. 
    # This will significantly improve the performance of the code. 
    # Also, instead of checking if '%' or ',' exists in each column, we can directly replace them. 
    # This is because the replace operation will not affect columns that do not contain these characters.
    # Moreover, the dropna() function in the original code is not necessary because the 'coerce' argument in to_numeric() function will replace non-numeric values with NaN.
    # Optimized code:
    # Replace '%' and ',' in all columns at once
