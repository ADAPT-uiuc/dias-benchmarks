...
parser.add_argument('--no_sliced_exec', action='store_true', help='Disable sliced execution in the rewriter.')
parser.add_argument('--rewr_stats', action='store_true', help='Gather stats from the rewriter.')

args = parser.parse_args()

# Some validation
msg=None
if args.no_sliced_exec and not args.rewriter:
  msg = "Disabling sliced execution makes sense only if the rewriter is enabled"
elif args.rewr_stats and not args.rewriter:
  msg = "Enabling rewriter stats makes sense only if the rewriter is enabled"
elif args.modin != -1 and args.modin < 2:
...
if args.rewr_stats:
  ...
  VER_sliced_exec = "sliced_exec_OFF" if args.no_sliced_exec else "sliced_exec_ON"
...
for nb in nbs:
  ...
  succ = run_nb.run_nb(full_path, args.rewriter, args.modin, args.less_replication, args.no_sliced_exec, args.rewr_stats)
  assert succ
